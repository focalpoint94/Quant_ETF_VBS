...
if __name__ == '__main__':
    try:
        symbol_list = get_ETF_list(num_ETF=9)
        bought_list = []                                            # 매수 완료된 종목 리스트
        target_buy_count = 6                                        # 매수할 종목 수
        buy_percent = 1 / target_buy_count * 0.99                   # 국내 주식형 ETF: 소득세 면제
        ...
        while True:
            ...
            t_end = t_now.replace(hour=14, minute=45, second=0, microsecond=0)
            ...
            if t_sell < t_now < t_exit:
                ...
                have_list = []
                for stock in stocks:
                    have_list.append(stock['code'])
                lossy_list = []
                for code in have_list:
                    _, _, _yield = get_stock_status(code)
                    if _yield < 0:
                        lossy_list.append(code)
                sell_stock_list(lossy_list)
                time.sleep(5)
            if t_exit < t_now:
                get_stock_balance('ALL')
                dbgout('거래 종료!')
